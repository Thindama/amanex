...
export default function BotControl({ lang, botOn, setBotOn }) {
    ...
    <div className="page-scroll">
      <div className="row-3">
        <div className="card" style={{display:'flex',flexDirection:'column',alignItems:'center',gap:'16px',padding:'24px 18px'}}>
          <div style={{width:'100%',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            ...
            <div className={`badge ${botOn?'b-green':'b-red'}`}>{botOn?t(lang,'aktiv'):t(lang,'botStopped')}</div>
          </div>
          <div className={`toggle-circle ${botOn?'tc-on':'tc-off'}`} onClick={() => setBotOn(!botOn)}>
            {botOn ? t(lang,'boton') : t(lang,'botoff')}
          </div>
          <div style={{textAlign:'center'}}>
            <div style={{fontSize:'15px',fontWeight:600}}>{botOn ? t(lang,'botRunning') : t(lang,'botStopped')}</div>
            <div style={{fontSize:'11px',color:'var(--muted)',fontFamily:'JetBrains Mono,monospace',marginTop:'4px'}}>
              {botOn ? 'Seit 4h 32min' : t(lang,'botStoppedMsg')}
            </div>
          </div>
        </div>
        ...
        <div className="card">
          ...
          <Slider label="Scan-Intervall" desc="Wie oft der Scanner laeuft" min={5} max={60} defaultVal={15} step={5} suffix=" Min" />
          <Slider label="Aktive Stunden" desc="Trading-Zeitfenster" min={1} max={24} defaultVal={24} step={1} suffix="h" />
        ...
        <div className="card">
          ...
          <div style={{textAlign:'center',padding:'10px 0 4px'}}>
            <div style={{fontSize:'32px',fontWeight:600,fontFamily:'JetBrains Mono,monospace',color:'var(--green)'}}>{kelly}</div>
      ...
      <div className="row-2">
        <div className="card">
          ...
          <Slider label="Min. Edge fuer Trade" desc="KI muss Marktpreis um X% uebertreffen" min={2} max={15} defaultVal={4} step={1} suffix="%" />
          <Slider label="Max. pro Trade" desc="Maximaler Einsatz pro Trade" min={1} max={10} defaultVal={5} step={1} suffix="%" />
          <Slider label="Max. gleichzeitige Trades" desc="Offene Positionen gleichzeitig" min={1} max={20} defaultVal={15} step={1} suffix="" />
          <Slider label="Tagesverlust-Limit" desc="Bot pausiert bei Ueberschreitung" min={5} max={30} defaultVal={15} step={1} suffix="%" />
          <Slider label="Max. Drawdown" desc="Alle Trades blockiert bei Ueberschreitung" min={2} max={20} defaultVal={8} step={1} suffix="%" />
        </div>
        ...
        <div className="card">
          ...
          <div className="row-2" style={{marginBottom:'16px'}}>
            {['Kalshi','Polymarket'].map(p => (
              <div key={p} style={{padding:'12px',borderRadius:'8px',border:'1px solid var(--blue)',background:'rgba(79,142,255,.07)',cursor:'pointer'}}>
                <div style={{fontSize:'13px',fontWeight:500}}>{p}</div>
                <div style={{fontSize:'10px',color:'var(--muted)',marginTop:'3px'}}>{p==='Kalshi'?'US-reguliert':'Crypto'}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'6px'}}>
            {MODELS.map(m => (
              <div key={m} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'8px 12px',background:'var(--surface)',borderRadius:'7px',border:'1px solid var(--border)'}}>
                ...
                <div className="badge b-green">{t(lang,'aktiv')}</div>
              </div>
            ))}
          </div>
          <button className={`save-btn ${saved?'saved':''}`} onClick={handleSave}>
            {saved ? t(lang,'saved') + ' ✓' : t(lang,'saveset')}
          </button>
        </div>
      </div>
    </div>
